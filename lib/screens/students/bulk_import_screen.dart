...
class _BulkImportScreenState extends State<BulkImportScreen> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('Import Mahasiswa dari Excel'),
        actions: [
      ...
      body: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
                    ...
                    children: [
                      Row(
                        ...
                        children: [
                          ...
                          Chip(
                            label: Text('${_students.length} mahasiswa'),
                      ...
                      Container(
                        ...
                        child: ListView.builder(
                          shrinkWrap: true,
                          itemCount:
                              _students.length > 5 ? 5 : _students.length,
                      ...
                      if (_students.length > 5)
                        Padding(
                          ...
                          child: Text(
                            'Menampilkan 5 dari ${_students.length} data...',
                            style: TextStyle(
                              fontSize: 12,
                              color: Colors.grey[600],
                              fontStyle: FontStyle.italic,
            ...
            // Import Button
            if (_students.isNotEmpty && _errors.isEmpty) ...[
              const SizedBox(height: 24),
              SizedBox(
                width: double.infinity,
                height: 48,
                child: ElevatedButton.icon(
                  ...
                  label: Text(_isLoading
                      ? 'Mengimpor...'
                      : 'Import ${_students.length} Mahasiswa'),
      ...
      if (mounted) {
        if (students.isNotEmpty) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content:
                  Text('Berhasil memuat ${students.length} data mahasiswa'),
  ...
  Future<void> _importStudents() async {
    ...
    try {
      final importErrors =
          await context.read<StudentProvider>().addStudentsBulk(_students);

      final successCount = _students.length - importErrors.length;

      if (mounted) {
        if (successCount > 0) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content: Text('Berhasil mengimpor $successCount mahasiswa'),
              backgroundColor: Colors.green,
      ...
      SnackBar(
        content: const Column(
          ...
          children: [
            ...
            Text('John Doe | Jl. Contoh No. 123, Jakarta | 081234567890'),
            Text('Jane Smith | Jl. Sample No. 456, Bandung | 087654321098'),
          ],
        ),
